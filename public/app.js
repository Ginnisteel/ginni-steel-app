(function () {
  ...
  function sizeInfoText(sizeObj) {
    if (!sizeObj) return "";
    return sizeObj.pcs != null ? sizeObj.pcs + " pcs per bundle" : "";
  }
  ...
  function renderCustomerView() {
    ...
    state.products.forEach(function (p) {
      var sizeOpts = p.sizes.map(function (s) {
        return '<option value="' + s.label + '" data-pcs="' + (s.pcs == null ? "" : s.pcs) + '">' + s.label + '</option>';
      }).join("");
    ...
    if (keys.length === 0) {
      ...
      keys.forEach(function (k) {
        ...
        html +=
          ...
          '  <div class="desc"><span class="lname">' + line.name + '</span> \u00d7 ' + line.bundles + ' bundle' + (line.bundles > 1 ? "s" : "") + pcsTxt + '<br><span class="lsize">' + line.size + '</span></div>' +
  ...
  function wireCustomerEvents() {
    app.querySelectorAll(".size-select").forEach(function (sel) {
      sel.addEventListener("change", function () {
        ...
        var pcs = opt.dataset.pcs === "" ? null : parseInt(opt.dataset.pcs, 10);
        var infoEl = document.getElementById("info-" + pid);
        if (infoEl) infoEl.textContent = sizeInfoText({ pcs: pcs });
      });
    });
    ...
    app.querySelectorAll(".add-btn").forEach(function (btn) {
      btn.addEventListener("click", function () {
        ...
        if (state.cart[key]) {
          ...
          state.cart[key] = { productId: pid, name: product.name, image: product.image, size: size, pcs: pcs, bundles: bundles };
        }
    ...
    if (submitBtn) submitBtn.addEventListener("click", function () {
      var items = Object.keys(state.cart).map(function (k) {
        var l = state.cart[k];
      ...
      api("/orders", { method: "POST", body: { items: items, note: state.note } })
        .then(function (res) {
          state.cart = {};
          state.note = "";
          showToast("Order " + res.orderCode + " submitted to the order desk");
          renderCustomerView();
        })
        .catch(function (err) {
  ...
  function renderOrdersTab() {
    ...
    if (state.orders.length === 0) {
      ...
      state.orders.forEach(function (o) {
        ...
        html += '    <div class="overflow-x"><table class="order-items"><thead><tr><th>Item</th><th>Size</th><th>Bundles</th><th>Pcs</th></tr></thead><tbody>';
        o.items.forEach(function (it) {
          var pcsTxt = it.pcs_per_bundle != null ? (it.pcs_per_bundle * it.bundles) : "\u2014";
          html += '<tr><td>' + it.product_name + '</td><td>' + it.size_label + '</td><td>' + it.bundles + '</td><td>' + pcsTxt + '</td></tr>';
        });
